...
bool TmxLevel::LoadFromFile(const std::string &filepath)
{
    ...
    while (layerElement)
    {
        ...
        {
            //added by me
            const int tileGID = (tileElement->Attribute("gid") != NULL) ? std::stoi(tileElement->Attribute("gid")) : 0;
    ...
    if (map->FirstChildElement("objectgroup") != nullptr)
    {
        ...
        while (objectGroupElement)
        {
            ...
            while (objectElement)
            {
                ...
                if (objectElement->Attribute("gid") == nullptr)//(objectElement->Attribute("width") != nullptr)
                {
