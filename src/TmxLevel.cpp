...
std::string TmxObject::GetPropertyString(const std::string &propertyName) { return properties[propertyName]; }
...
bool TmxLevel::LoadFromFile(const std::string &filepath) {
    ...
    while (layerElement) {
        ...
        while (tileElement) {
            // added by me
            const int tileGID =
                (tileElement->Attribute("gid") != NULL) ? std::stoi(tileElement->Attribute("gid")) : 0;
        ...
        while (objectGroupElement) {
            ...
            while (objectElement) {
                ...
                if (objectElement->Attribute("gid") ==
                    nullptr)  //(objectElement->Attribute("width") != nullptr)
                {
...
sf::Vector2i TmxLevel::GetTileSize() const { return sf::Vector2i(m_tileWidth, m_tileHeight); }

float TmxLevel::GetTilemapWidth() const { return static_cast<float>(m_tileWidth * m_width); }

float TmxLevel::GetTilemapHeight() const { return static_cast<float>(m_tileHeight * m_height); }

sf::Vector2f TmxLevel::GetTilemapSize() const { return sf::Vector2f(GetTilemapWidth(), GetTilemapHeight()); }
